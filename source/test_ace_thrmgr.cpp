...
static void* worker(intptr_t iterations)
{
...
static const intptr_t DEFAULT_ITERATIONS = 10000;
...
int ACE_TMAIN(int argc, ACE_TCHAR * argv[])
{
	...
	int grp_id = thr_mgr->spawn_n(n_threads, ACE_THR_FUNC(worker), reinterpret_cast<void*>(n_iterations), THR_NEW_LWP | THR_DETACHED);

	//wait for 1 second and the suspend every thread in the group
	ACE_OS::sleep(1);
	...
	//wait for 1 more second and then resume every thread in the group
	...
	//wait for 1 more secondd and the send a SIGINT to every thread in the group
	ACE_OS::sleep(ACE_Time_Value(1));
